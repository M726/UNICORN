...
C OPEN OPEN OPEN OPEN OPEN OPEN OPEN  OPEN OPEN OPEN OPEN OPEN OPEN OPEN
           OPEN(16,FILE=FILE6,ACCESS='SEQUENTIAL',STATUS='UNKNOWN')
      IF(NEVOL.GE.1)
     1     OPEN(14,FILE=FILETM,ACCESS='SEQUENTIAL',STATUS='UNKNOWN')
      IF(ISTORE.EQ.1.OR.ISTORE.LE.ITEND) 
     1     OPEN(12,FILE=FILEOT,ACCESS='SEQUENTIAL',STATUS='UNKNOWN')
C--NEW-NEW-NEW-NEW-NEW-NEW-NEW-NEW-NEW-NEW-NEW-NEW-NEW-NEW-NEW-NEW-NEW--
      IF(ISTORE.EQ.1.OR.ISTORE.LE.ITEND) 
...
C--NEW-NEW-NEW-NEW-NEW-NEW-NEW-NEW-NEW-NEW-NEW-NEW-NEW-NEW-NEW-NEW-NEW--
      IF(NOPR.GT.0)
     1  OPEN(20,FILE=FILEPT,ACCESS='SEQUENTIAL',STATUS='UNKNOWN')
      IF(IREAD.GE.1) 
     1     OPEN(32,FILE=FILEIN,ACCESS='SEQUENTIAL',STATUS='OLD')
      IF(NDRV.GE.1) 
     1     OPEN(72,FILE=FILEDR,ACCESS='SEQUENTIAL',STATUS='UNKNOWN')
      IPAGE=1
      IF(ISANM.LT.0) THEN
                     ISANM=-ISANM
                     IPAGE=0
      ...
      IF((KORNT.EQ.2).OR.(KORNT.EQ.4)) THEN
                 ...
                 END IF
      IF(NBAVE.GE.1) 
     1      OPEN(13,FILE=FILEAV,ACCESS='SEQUENTIAL',STATUS='UNKNOWN')
...
C-----------------------------------------------------------------------
  ...
  267 CONTINUE
      ...
      IF(MOD(ITR-1,IPRES*20).EQ.0.OR.ITR.EQ.1) WRITE(*,754) 
     1       ITR,IA,DTMS,TIMEAVERAGE,TIMEREMAINING
  754 FORMAT(1X,13('-'),' ITERATIONS FROM ',I5,' TO ',I5,
     1       2X,'(DT = ',F8.6,' ms) ',13('-')/,
     2       1X,'Avg Time: ', F10.2,
     3       3X,'Remaining: ', F10.1/,
     4       2X,'U-RESID            M-RESID   HT-RESID     CHEM-RES',
     5       6X,'K & EPS-RESID',8X,'TIME(s)')


      IF(MOD(ITR,IPRES).EQ.0.OR.MOD(ITR,100).EQ.0) WRITE(*,756) 
     1        RESID,IRLXU,IRLXV,IRLXW,
     2        RESDM,RESDH,IRLXH,RESDC,IRLXSP,RESDK,RESDE,IRLXKE,
     3        DELTATIME
  756 FORMAT(1X,1PE8.1,'(',I2,',',I2,',',I2,') ',1PE8.1,1X,
     1  1PE8.1,'(',I2,') ',1PE8.1,'(',I2,') ',
     2  1PE8.1,' &',1PE8.1,'(',I2,')',2X, F0.3)
  ...
  758 FORMAT(1X,'N=',I4,', DT=',F8.6,'ms, TIME=',F9.1,'s'/
     ...
     3  'H-RES=',1PE9.2,'(',I2,') ','; CHEM-RES=',1PE9.2,'(',I2,')'/
     4  'K & EPS-RES=',1PE9.2,' &',1PE9.2,'(',I2,')')
